...
export default function RechargePage() {
  ...
  const estimatedMinutes = config
    ...
    : null;
  const exampleTaskCost = calculateEstimatedCredits(120, config);

  const copyRequest = async () => {
    const request = [
      'SchemaBio SaaS Credit 充值申请',
    ...
    <PageContent className="yj-page-shell">
      <div className="yj-page-header">
        <div>
          ...
          <p className="yj-page-subtitle">选择 Credit 数量并生成充值申请，平台确认后余额会显示在费用中心。</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 gap-5 lg:grid-cols-[minmax(0,1fr)_320px]">
        <section className="yj-panel p-5">
          <div className="mb-5 flex items-center gap-2">
            ...
            <h3 className="text-base font-medium text-fg-default">充值 Credit</h3>
          </div>
          ...
          <div className="mt-6 flex items-center justify-between gap-4 border-t border-border-default pt-5">
            <div>
              ...
              <p className="text-2xl font-semibold text-fg-default">{amount} Credit</p>
            </div>
            <Button variant="primary" onClick={() => void copyRequest()}>
        ...
        <aside className="space-y-4">
          <div className="yj-panel p-4">
            ...
            <p className="mt-2 text-2xl font-semibold text-fg-default">{balance?.balance ?? '--'} Credit</p>
          </div>
          <div className="yj-panel p-4">
            <div className="flex items-center gap-2 text-fg-muted">
            ...
            <p className="mt-2 text-xs text-fg-muted">120 分钟任务当前预计扣费 {exampleTaskCost ?? '--'} Credit</p>
          </div>

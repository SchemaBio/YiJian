...
export function ReportTab({ taskId }: ReportTabProps) {
  ...
  const handleStatusChange = (record: ReportRecord, action: string) => {
    setStatusAction({ record, action });
    setStatusModalOpen(true);
  };

  const confirmStatusChange = async () => {
    if (!statusAction) return;
    const { record, action } = statusAction;
  ...
  const getActionButtons = (record: ReportRecord) => {
    ...
    // 预览按钮
    buttons.push(
      <Button key="preview" variant="ghost" size="small" onClick={() => handlePreview(record)} title="预览">
    ...
    if (record.status !== 'RELEASED') {
      buttons.push(
        <Button key="delete" variant="ghost" size="small" onClick={() => handleDelete(record)} title="删除">
  ...
  return (
    <div className="flex gap-4">
            ...
            <div className="border border-border rounded-lg overflow-hidden">
              <table className="w-full text-sm table-fixed">
                <thead className="bg-canvas-subtle border-b border-border">
                  <tr>
                    <th className="text-left px-4 py-2 font-medium text-fg-muted w-[25%]">报告名称</th>
                    <th className="text-center px-4 py-2 font-medium text-fg-muted w-[10%]">类型</th>
                    <th className="text-center px-4 py-2 font-medium text-fg-muted w-[10%]">状态</th>
                    <th className="text-center px-4 py-2 font-medium text-fg-muted w-[15%]">创建时间</th>
                    <th className="text-center px-4 py-2 font-medium text-fg-muted w-[10%]">创建者</th>
                    <th className="text-center px-4 py-2 font-medium text-fg-muted w-[30%]">操作</th>
                  </tr>
                </thead>
                <tbody>
                  {records.map((record, index) => (
                    <tr
                      key={record.id}
                      className={`${index !== records.length - 1 ? 'border-b border-border' : ''} cursor-pointer hover:bg-canvas-subtle ${previewRecord?.id === record.id ? 'bg-accent-subtle' : ''}`}
                    ...
                    >
                      ...
                      <td className="px-4 py-3 text-center">
                        <Tag variant={record.type === 'generated' ? 'info' : 'neutral'}>
                          {record.type === 'generated' ? '生成' : '上传'}
                        </Tag>
                      </td>
                      <td className="px-4 py-3 text-center">
                        <Tag variant={STATUS_CONFIG[record.status].variant}>
      ...
      {previewRecord && (
        <>
          <div className="fixed inset-0 z-10" onClick={closePreview} />
          <div
          ...
          >
            <div className="flex items-center justify-between px-4 py-3 border-b border-border bg-canvas-subtle rounded-t-lg">
              <div className="flex items-center gap-2">
                ...
                <Tag variant={STATUS_CONFIG[previewRecord.status].variant} className="ml-2">
                  {STATUS_CONFIG[previewRecord.status].label}
                </Tag>
              </div>
              <div className="flex items-center gap-2">
      ...
      {/* 状态变更确认弹窗 */}
      <Modal open={statusModalOpen} onOpenChange={setStatusModalOpen}>
        ...
        <ModalFooter>
          ...
          <Button variant="primary" onClick={confirmStatusChange}>确认</Button>
        </ModalFooter>
      </Modal>
    </div>
  );
